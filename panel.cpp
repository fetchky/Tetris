	...
	ui(new Ui::Panel)
{
	...
	//m_mediaPlayer->setMedia(QUrl::fromLocalFile(QString("C:/Users/Administrator/Documents/test230/ImageclearRow.mp3")));
...
void Panel::setbackground(const int &Lv)
{
	...
	case LV0:
		...
		ui->label_gameLevel->setText(QL("俄罗斯方块"));
		m_pixmap.load("://Image/background.png");
		break;
	case LV1:
		m_currentLv = LV1;
		m_currentLVSpeed = LV1_SPEED;
		ui->label_gameLevel->setText(QL("第一关"));
		m_pixmap.load(":/Image/background01.png");
		break;
	case LV2:
		m_currentLv = LV2;
		m_currentLVSpeed = LV2_SPEED;
		ui->label_gameLevel->setText(QL("第二关"));
		m_pixmap.load(":/Image/background02.png");
		break;
	case LV3:
		m_currentLv = LV3;
		m_currentLVSpeed = LV3_SPEED;
		ui->label_gameLevel->setText(QL("第三关"));
		m_pixmap.load(":/Image/background03.png");
		break;
	case LV4:
		m_currentLv = LV4;
		m_currentLVSpeed = LV4_SPEED;
		ui->label_gameLevel->setText(QL("第四关"));
		m_pixmap.load(":/Image/background04.png");
		break;
	case LV5:
		m_currentLv = LV5;
		m_currentLVSpeed = LV5_SPEED;
		ui->label_gameLevel->setText(QL("第五关"));
		m_pixmap.load(":/Image/background05.png");
		break;
	default:
		break;
	}
...
void Panel::on_pbt_pauseGame_clicked()
{
	...
	m_currentBox->slot_stopTimer();
	ui->pbt_startGame->setEnabled(true);
	QMessageBox::information(this, QL("提示"), QL("暂停游戏！"),QMessageBox::Yes);
}
...
//    qDebug() << "game over";
	m_isGameOver = true;
	QMessageBox::information(this,QL("提示"),QL("您的游戏得分为%1!").arg(ui->LCDNum_Score->value()),QMessageBox::Yes);
	ui->LCDNum_Score->display(0);
